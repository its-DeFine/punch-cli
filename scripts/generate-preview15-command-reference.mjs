...
const format = JSON.parse(fs.readFileSync(path.join(repo, 'docs/schemas/preview15-public-command-contract-format.v1.json'), 'utf8'));
const marker = /<!-- GENERATED PREVIEW15 COMMAND REFERENCE:BEGIN -->[\s\S]*?<!-- GENERATED PREVIEW15 COMMAND REFERENCE:END -->/;
...
const expectedPrivateRuntimeBinding = Object.freeze({
  commit: '7867a5f101180b231e24d2b87bc4e86ef90b9b38',
  tree: '7ded34759e139fb54b3f15fabf8992bdd3943628',
  controlArchiveSha256: 'sha256:11271fd5454c993b6389af9c1833d0e9c380b2ea7fd6ffde731d02cec4cda0e3'
});
...
function validateTemplate(template) {
  ...
  for (const key of format.artifactKeys) candidate.artifact[key] = sha256(`preview15-template-${key}`);
...
function verifyBinding(contract, args) {
  ...
  assert.equal(runtime.schemaVersion, 'punch.preview15-runtime-contract.v1', 'runtime contract schema mismatch');
...
function update(target, generated, write) {
  ...
  if (!write) {
    assert.equal(next, current, 'generated Preview.15 command reference is stale; rerun with --write');
    return;
  }
...
function fixture() {
  const candidate = structuredClone(load(path.join(repo, 'docs/preview15-public-command-contract.template.json')));
  for (const key of format.artifactKeys) candidate.artifact[key] = sha256(`preview15-fixture-${key}`);
  candidate.artifact.packagedCliSurfaceSha256 = commandSurfaceDigest(candidate);
  return candidate;
}
...
function selfTest() {
  ...
  const archive = path.join(directory, 'punch-cli-0.1.0-preview.15-linux-x64.tar.gz');
  const sums = path.join(directory, 'SHA256SUMS');
  const runtime = path.join(directory, 'preview15-runtime-contract.json');
  ...
  fs.writeFileSync(runtime, JSON.stringify({
    schemaVersion: 'punch.preview15-runtime-contract.v1',
  ...
  process.stdout.write('Preview.15 generated command-reference drift gate negative fixtures: PASS\n');
}
...
try {
  ...
  if (args.selfTest && !args.contract && !args.target && !args.write && !args.verifyBinding) {
    ...
    process.stdout.write('Preview.15 command-contract template: valid and artifact binding pending\n');
    ...
    process.stdout.write(`Preview.15 generated command reference: ${args.write ? 'updated' : 'current'}\n`);
  } else {
    throw new Error('usage: generate-preview15-command-reference.mjs --template TEMPLATE.json | --contract CONTRACT.json --target REFERENCE.md [--write] [--verify-binding --archive ARCHIVE --sha256sums SHA256SUMS --runtime-contract RUNTIME.json] | --self-test');
  }
} catch (error) {
  process.stderr.write(`Preview.15 command reference: FAIL: ${error.message}\n`);
  process.exit(1);
}
