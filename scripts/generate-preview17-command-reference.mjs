...
const format = JSON.parse(fs.readFileSync(path.join(repo, 'docs/schemas/preview17-public-command-contract-format.v1.json'), 'utf8'));
const marker = /<!-- GENERATED PREVIEW17 COMMAND REFERENCE:BEGIN -->[\s\S]*?<!-- GENERATED PREVIEW17 COMMAND REFERENCE:END -->/;
...
const expectedPrivateRuntimeBinding = Object.freeze({
  ...
  controlArchiveSha256: 'sha256:33fcb27b312c346540075df64a8598133eb32b1bdce81378cb3b22026d5f8d1e'
});
...
function validateStaticReleaseSurface(contract) {
  ...
  assert.deepEqual(contract.workflows.provider,
    ['prepare-host', 'identity-init', 'onboarding-request', 'onboarding-status', 'join', 'overview', 'doctor', 'setup', 'service-status', 'offer-status'],
...
function validateFormat() {
  exactKeys(format, formatKeys, 'Preview.17 command-contract format');
  assert.equal(format.schemaVersion, 'punch.preview17-public-command-contract-format.v1');
  ...
  assert.equal(format.contractSchema, 'punch.preview17-public-command-contract.v1');
...
function validate(contract) {
  exactKeys(contract, format.requiredTopLevelKeys, 'Preview.17 command contract');
  assert.equal(contract.schemaVersion, 'punch.preview17-public-command-contract.v1');
  assert.equal(contract.releaseVersion, '0.1.0-preview.17');
...
function validateTemplate(template) {
  ...
  for (const key of format.artifactKeys) candidate.artifact[key] = sha256(`preview17-template-${key}`);
...
function verifyBinding(contract, args) {
  ...
  assert.equal(runtime.schemaVersion, 'punch.preview17-runtime-contract.v1', 'runtime contract schema mismatch');
...
function update(target, generated, write) {
  ...
  if (!write) {
    assert.equal(next, current, 'generated Preview.17 command reference is stale; rerun with --write');
    return;
  }
...
function fixture() {
  const candidate = structuredClone(load(path.join(repo, 'docs/preview17-public-command-contract.template.json')));
  for (const key of format.artifactKeys) candidate.artifact[key] = sha256(`preview17-fixture-${key}`);
  candidate.artifact.packagedCliSurfaceSha256 = commandSurfaceDigest(candidate);
  return candidate;
}
...
function selfTest() {
  ...
  const archive = path.join(directory, 'punch-cli-0.1.0-preview.17-linux-x64.tar.gz');
  const sums = path.join(directory, 'SHA256SUMS');
  const runtime = path.join(directory, 'preview17-runtime-contract.json');
  ...
  fs.writeFileSync(runtime, JSON.stringify({
    schemaVersion: 'punch.preview17-runtime-contract.v1',
  ...
  process.stdout.write('Preview.17 generated command-reference drift gate negative fixtures: PASS\n');
}
...
try {
  ...
  if (args.selfTest && !args.contract && !args.target && !args.write && !args.verifyBinding) {
    ...
    process.stdout.write('Preview.17 command-contract template: valid and artifact binding pending\n');
    ...
    process.stdout.write(`Preview.17 generated command reference: ${args.write ? 'updated' : 'current'}\n`);
  } else {
    throw new Error('usage: generate-preview17-command-reference.mjs --template TEMPLATE.json | --contract CONTRACT.json --target REFERENCE.md [--write] [--verify-binding --archive ARCHIVE --sha256sums SHA256SUMS --runtime-contract RUNTIME.json] | --self-test');
  }
} catch (error) {
  process.stderr.write(`Preview.17 command reference: FAIL: ${error.message}\n`);
  process.exit(1);
}
