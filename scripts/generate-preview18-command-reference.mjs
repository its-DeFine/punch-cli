...
const format = JSON.parse(fs.readFileSync(path.join(repo, 'docs/schemas/preview18-public-command-contract-format.v1.json'), 'utf8'));
const marker = /<!-- GENERATED PREVIEW18 COMMAND REFERENCE:BEGIN -->[\s\S]*?<!-- GENERATED PREVIEW18 COMMAND REFERENCE:END -->/;
...
const expectedPrivateRuntimeBinding = Object.freeze({
  ...
  controlArchiveSha256: 'sha256:fcaa8d0c28d48f68bf940811457cfcd2ff594c9d14139dae33301749d6c0ae5a'
});
...
function validateStaticReleaseSurface(contract) {
  requireFlags(contract, 'provider', 'prepare-host', ['--machine-id', '--plan-digest', '--install-dependencies', '--yes']);
  requireFlags(contract, 'provider', 'onboarding-request', ['--provider-label', '--idempotency-key', '--cpu-cores', '--gpu-units', '--ram-mib', '--disk-gib']);
  requireFlags(contract, 'provider', 'onboarding-status', ['--request-ref']);
  requireFlags(contract, 'provider', 'overview', ['--credential-file']);
  ...
  requireFlags(contract, 'provider', 'offer-list', ['--machine-id', '--state-dir', '--agent-config']);
  requireFlags(contract, 'provider', 'offer-replace', [
    '--machine-id', '--state-dir', '--agent-config', '--offer-id', '--idempotency-key', '--yes'
  ]);
  assert.deepEqual(contract.workflows.provider,
    ['prepare-host', 'identity-init', 'onboarding-request', 'onboarding-status', 'join', 'overview', 'doctor', 'setup',
      'service-status', 'offer-list', 'offer-status', 'offer-unlist', 'offer-retire', 'offer-replace'],
...
function validateFormat() {
  ...
  assert.deepEqual(format.securityKeys, [
    ...
    'appendOnlyProviderOnboarding', 'sequentialOfferReplacementOnly',
    'guidedTargetedCanonicalZeroOnly', 'scopedBuyerSshEgressConsentRequired',
    'guidedSshSpawnsChild', 'osc52ClipboardExplicitConsentRequired',
    'maxAuthorizedWindowSeconds', 'ineligibleOffersOrderable', 'paymentSettlementEnabled'
  ]);
}
...
function validate(contract) {
  ...
  assert.equal(contract.security.appendOnlyProviderOnboarding, true);
  assert.equal(contract.security.sequentialOfferReplacementOnly, true);
  assert.equal(contract.security.guidedTargetedCanonicalZeroOnly, true);
  assert.equal(contract.security.scopedBuyerSshEgressConsentRequired, true);
  assert.equal(contract.security.guidedSshSpawnsChild, false);
  assert.equal(contract.security.osc52ClipboardExplicitConsentRequired, true);
...
function validateTemplate(template) {
  ...
  for (const key of format.artifactKeys) candidate.artifact[key] = sha256(`preview18-template-${key}`);
...
function markdown(contract) {
  ...
  lines.push('## Security boundary', '', '- Linux/x64 only.', '- Privileged dependency changes require explicit confirmation; direct non-interactive installation requires cached sudo.', '- Provider onboarding appends distinct approved authority and cannot replace existing Provider or Buyer authority.', '- Provider replacement is sequential: one nonterminal offer per machine.', '- Guided Buyer ordering accepts only an eligible targeted canonical numeric +0 offer.', '- Guided SSH preparation requires exact scoped egress consent, prints a copy-ready command, and never spawns SSH.', '- OSC 52 clipboard delivery is explicit-consent-only and retains the visible fallback.', '- Authorized access windows are capped at 259200 seconds.', '- Payment settlement is disabled for this preview.', '', '<!-- GENERATED PREVIEW18 COMMAND REFERENCE:END -->');
  return `${lines.join('\n')}\n`;
}
...
function verifyBinding(contract, args) {
  ...
  assert.equal(runtime.schemaVersion, 'punch.preview18-runtime-contract.v1', 'runtime contract schema mismatch');
...
function update(target, generated, write) {
  ...
  if (!write) {
    assert.equal(next, current, 'generated Preview.18 command reference is stale; rerun with --write');
    return;
  }
...
function fixture() {
  const candidate = structuredClone(load(path.join(repo, 'docs/preview18-public-command-contract.template.json')));
  for (const key of format.artifactKeys) candidate.artifact[key] = sha256(`preview18-fixture-${key}`);
  candidate.artifact.packagedCliSurfaceSha256 = commandSurfaceDigest(candidate);
  return candidate;
}
...
function selfTest() {
  ...
  for (const mutate of [
    ...
    (contract) => { contract.privateRuntimeBinding.commit = '1111111111111111111111111111111111111111'; },
  ...
  const archive = path.join(directory, 'punch-cli-0.1.0-preview.18-linux-x64.tar.gz');
  const sums = path.join(directory, 'SHA256SUMS');
  const runtime = path.join(directory, 'preview18-runtime-contract.json');
  ...
  fs.writeFileSync(runtime, JSON.stringify({
    schemaVersion: 'punch.preview18-runtime-contract.v1',
  ...
  process.stdout.write('Preview.18 generated command-reference drift gate negative fixtures: PASS\n');
}
...
try {
  ...
  if (args.selfTest && !args.contract && !args.target && !args.write && !args.verifyBinding) {
    selfTest();
  } else if (!args.selfTest && args.template && !args.contract && !args.verifyBinding) {
    const template = load(path.resolve(args.template));
    validateTemplate(template);
    if (args.target) {
      update(path.resolve(args.target), markdown(template), args.write);
      process.stdout.write(`Preview.18 generated gated command reference: ${args.write ? 'updated' : 'current'}\n`);
    } else {
      assert.equal(args.write, false, '--write requires --target');
      process.stdout.write('Preview.18 command-contract template: valid and artifact binding pending\n');
    }
    ...
    process.stdout.write(`Preview.18 generated command reference: ${args.write ? 'updated' : 'current'}\n`);
  } else {
    throw new Error('usage: generate-preview18-command-reference.mjs --template TEMPLATE.json [--target REFERENCE.md [--write]] | --contract CONTRACT.json --target REFERENCE.md [--write] [--verify-binding --archive ARCHIVE --sha256sums SHA256SUMS --runtime-contract RUNTIME.json] | --self-test');
  }
} catch (error) {
  process.stderr.write(`Preview.18 command reference: FAIL: ${error.message}\n`);
  process.exit(1);
}
