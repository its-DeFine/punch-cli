...
const schema = JSON.parse(fs.readFileSync(path.join(repo, 'docs/schemas/preview18-clean-host-e2e-report.v1.json'), 'utf8'));
...
function validate(report, receipt, receiptBytes) {
  ...
  assert.equal(report.releaseVersion, '0.1.0-preview.18');
...
function selfTest() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'punch-preview18-gate-'));
  ...
  process.stdout.write('Preview.18 clean-host E2E release gate negative fixtures: PASS\n');
}
...
function parseFiles(args) {
  assert.equal(args.length, 4, 'usage: verify-preview18-clean-host-e2e.mjs --report REPORT.json --receipt RECEIPT.json | --self-test');
  const files = {};
...
try {
  ...
  if (args.length === 1 && args[0] === '--self-test') {
    ...
    process.stdout.write('Preview.18 clean-host E2E release gate: PASS\n');
  }
} catch (error) {
  process.stderr.write(`Preview.18 clean-host E2E release gate: FAIL: ${error.message}\n`);
  process.exit(1);
}
