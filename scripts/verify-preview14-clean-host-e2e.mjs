...
function selfTest() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'punch-preview14-gate-'));
...
function parseFiles(args) {
  assert.equal(args.length, 4, 'usage: verify-preview14-clean-host-e2e.mjs --report REPORT.json --receipt RECEIPT.json | --self-test');
  const files = {};
