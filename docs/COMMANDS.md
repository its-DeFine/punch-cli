...
`offer-create` requires explicit CPU/GPU/VRAM/RAM/disk capacity flags.
`offer-create`, `offer-unlist`, `offer-retire`, and `offer-replace` require a
stable `--idempotency-key`; create and replacement also require explicit `--yes`
for service reuse/start. The complete lifecycle and archive-release gate are in
