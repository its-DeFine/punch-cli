...
| `offer-create` | Create and activate a new offer within the machine's admin-assigned slot and verified resource limits |
...
Initial Provider setup creates the first non-discoverable `PENDING_AGENT` offer.
After enrollment, `offer-create` creates additional offers within the
admin-assigned slot limit and the machine's verified aggregate CPU, GPU, VRAM,
RAM, and disk capacity. Both paths activate `LISTED` only after a fresh signed
heartbeat. Unlisting hides an offer but retains its allocation; retiring it
releases that offer's slot and resources while the machine remains ready.
...
canonical `STATE_DIR/provider-agent.json` generated by setup. `--agent-config`
is an optional advanced exact-match override. Commands acting on one offer also
...
`offer-unlist` and `offer-retire` derive stable operation identities from the
machine, offer, and action. `--idempotency-key` remains an optional advanced
...
- Provider service actions return `schemaVersion`, `action`, `unit`, `state`,
  and bounded `output`. Offer create returns `schemaVersion`, `machineId`,
  `offerId`, `state`, and `replayed`. Offer status returns `schemaVersion`, `offerId`,
...
For `offer-create`, rerun the same command and inputs after an ambiguous result;
the CLI resumes the pending operation automatically. A different create intent
is rejected until that pending operation reaches an authoritative result.
